...
export default class Validator {
   ...
   * @param type 数据类型
   */
  constructor(fieldName: string, trigger: IValidatorTrigger, type: IValidatorType) {
   ...
   */
  static builder(fieldName: string, trigger: IValidatorTrigger = 'change', type: IValidatorType = 'string') {
        ...
        this.add({
          validator: (_rule: any, value: any, callback: Function) => {
            ...
            if (decimalValue.length != decimalLength) {
              return callback(new Error(`${this._fieldName}需要保留${decimalLength}位小数`))
            }
            callback()
          },
        })
